...
function runGridWildDelayedIdleTask(callback, delay = 700) {
  window.setTimeout(() => runGridWildIdleTask(callback, 250), Math.max(0, delay));
}
...
function applyGridWildBootstrapData(data) {
  ...
  window.__gwState.onlineBootstrapDeferred = false;
  ...
  runGridWildIdleTask(() => {
    ...
    if (window.__gwState.activeQuestId) {
      ensureQuestDataLoaded().catch((err) => warnGridWildLoadFailure("Idle quest load failed:", err));
    }
  }, 900);
}

function startGridWildOnlineBootstrap(options = {}) {
  if (gwOnlineBootstrapPromise && options.force !== true) return gwOnlineBootstrapPromise;

  window.__gwState = window.__gwState || {};
  window.__gwState.onlineBootstrapDeferred = false;
  window.__gwState.onlineBootstrapStartedAt = Date.now();

  gwOnlineBootstrapPromise = window.GridWildAPI.getBootstrap({
    applySession: true,
    force: options.force === true,
    timeoutMs: options.timeoutMs || GRIDWILD_ONLINE_BOOT_TIMEOUT_MS
  })
    .then((data) => {
      applyGridWildBootstrapData(data);
      return data;
    })
    .catch((err) => {
      if (!isGridWildReloginRequiredError(err) && err?.code !== "GRIDWILD_API_TIMEOUT") {
        console.warn("Deferred online bootstrap unavailable:", err);
      }
      dispatchGridWildBootstrapUnavailable(err);
      return null;
    })
    .finally(() => {
      gwOnlineBootstrapPromise = null;
    });

  return gwOnlineBootstrapPromise;
}

function scheduleGridWildOnlineBootstrap() {
  if (gwOnlineBootstrapScheduled) return;
  gwOnlineBootstrapScheduled = true;
  ...
  afterPaint(() => {
    runGridWildDelayedIdleTask(() => startGridWildOnlineBootstrap(), GRIDWILD_ONLINE_BOOT_DELAY_MS);
  });
}
...
async function ensurePlayerBootstrapDetailsLoaded(options = {}) {
  ...
  gwPlayerDetailsPromise = (async () => {
    ...
    window.dispatchEvent(
      new CustomEvent("gwBootstrapDetailsReady", {
...
async function ensureQuestDataLoaded(options = {}) {
  window.__gwState = window.__gwState || {};
  if (window.__gwState.questDataLoaded && options.force !== true)
  ...
  gwQuestDataPromise = (async () => {
    ...
    const activeQuest = window.GridWildQuests?.getVisibleQuests?.()?.find(
      (q) => String(q.dbId || q.id) === String(window.__gwState.activeQuestId)
    );
    ...
    window.dispatchEvent(
      new CustomEvent("gwQuestDataReady", {
...
async function ensureSurveyDataLoaded(options = {}) {
  window.__gwState = window.__gwState || {};
  if (window.__gwState.surveyDataLoaded && options.force !== true)
  ...
  gwSurveyDataPromise = (async () => {
    ...
    window.dispatchEvent(
      new CustomEvent("gwSurveyDataReady", {
...
document.getElementById("btnMe").addEventListener("click", () => toggleSheet("me"));

document.getElementById("btnInfo").addEventListener("click", () => toggleSheet("info"));

document.getElementById("btnCommunity").addEventListener("click", () => toggleSheet("community"));

document.getElementById("btnQuest").addEventListener("click", () => toggleSheet("quest"));

document.getElementById("legendPill").addEventListener("click", () => toggleSheet("legend"));
...
window.GridWildHudLegend = (function () {
    ...
    timeconfidence: [
      {
        ...
        species: 7,
        observers: 2,
        last_observed: daysAgo(2400),
        median_last10_observed: daysAgo(2600)
      },
      {
        count: 18,
        species: 12,
        observers: 3,
        last_observed: daysAgo(30),
        median_last10_observed: daysAgo(2300)
      },
      {
        count: 28,
        species: 20,
        observers: 5,
        last_observed: daysAgo(25),
        median_last10_observed: daysAgo(90)
      }
    ],
    revisit: [
    ...
    reactivated: [
      {
        ...
        species: 7,
        observers: 2,
        last_observed: daysAgo(1800),
        median_last10_observed: daysAgo(2100)
      },
      {
        count: 18,
        species: 12,
        observers: 3,
        last_observed: daysAgo(80),
        median_last10_observed: daysAgo(900)
      },
      {
        count: 28,
        species: 20,
        observers: 5,
        last_observed: daysAgo(20),
        median_last10_observed: daysAgo(2400)
      }
    ],
    seasonalnow: [
  ...
  function refresh() {
    ...
    if (styles.length >= 3) {
      $("gwHudLegendBar").style.background =
...
window.GridWildHudTaxaFilter = (function () {
  ...
  function sync() {
    ...
    const nicheChecked =
      window.GridWildLocalNiches?.isVisible?.() ?? window.__gwState?.showLocalNiches !== false;
    if (niche) {
    ...
    const nightMode =
      (window.GridWildMapMode?.getMode?.() || window.__gwState?.dayNightMode) === "night";
    if (dayNight) {
  ...
  function toggleSurvey() {
    const real = surveyCheckbox();
    const current = real ? real.checked === true : window.__gwState?.showSurveyView !== false;
    const next = !current;
  ...
  function toggleNicheLayer() {
    ...
    const next =
      window.GridWildLocalNiches?.toggleVisible?.() ??
  ...
  function toggleNicheSparkles() {
    window.__gwState = window.__gwState || {};
    const next =
      window.GridWildCellSeededNiches?.toggleSparkles?.({ silent: true }) ??
...
function bindLensCards() {
  document.querySelectorAll(".gw-lens-card").forEach((card) => {
    card.onclick = () => {
      ...
      document.querySelectorAll(".gw-lens-card").forEach((c) => c.classList.remove("is-active"));
