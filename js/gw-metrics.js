window.GWMetrics = (() => {
  ...
  function buildSquareMetrics(square) {
    ...
    const dominant = Object.entries(iconic).sort((a, b) => b[1] - a[1])[0]?.[0] || "Unknown";
    ...
    return {
      ...
      median_last10_observed:
        square?.median_last10_observed || dateIsoFromMs(medianLast10ObservedMs),
      last_observed_ms: lastObservedMs || 0,
      median_last10_observed_ms: medianLast10ObservedMs || 0,

      activity_score: Math.log1p(count) * (1 + genera.size * 0.05)
    };
  }
  ...
  function buildChunkMetrics(chunk) {
    ...
    const pct = (v, p) => v[Math.floor((v.length - 1) * p)] || 0;

    return {
      nSquares: arr.length,
      p90Count: pct(counts, 0.9),
      p90Species: pct(spp, 0.9),
  ...
  function mergeSquareMetrics(records) {
    ...
    for (const rec of records) {
      ...
      const medianMs =
        Number(m.median_last10_observed_ms) || parseDateMs(m.median_last10_observed) || 0;
      ...
      for (const [k, v] of Object.entries(m.iconic_counts || {})) {
        merged.iconic_counts[k] = (merged.iconic_counts[k] || 0) + (v || 0);
      }
    ...
    const dom = Object.entries(merged.iconic_counts).sort((a, b) => b[1] - a[1])[0];

    merged.dominant_iconic = dom ? dom[0] : "Unknown";
