...
(function () {
  ...
  function savePatches() {
    ...
    window.dispatchEvent(new CustomEvent("gwPatchesChanged", {
  ...
  function patchCompleteness(patch, rings = patchRings(patch)) {
    ...
    const bboxCells =
      (bounds.maxIx - bounds.minIx + 1) *
      (bounds.maxIy - bounds.minIy + 1);
  ...
  function topBoundaryLabelLatLng(rings = []) {
    ...
    const lng = topPoints.reduce((sum, point) => sum + point.lng, 0) / Math.max(1, topPoints.length);
  ...
  function addPatchPolygon(patch, points, home) {
    ...
    const target = L.polygon(points.map(p => [p.lat, p.lng]), {
      pane: PANE,
      color: "#ffd85a",
      opacity: 0.96,
      weight: home ? 4 : 3.5,
      fillColor: "#ffd85a",
      fillOpacity: home ? 0.16 : 0.10,
      dashArray: home ? "" : "9 6",
      interactive: true,
      bubblingMouseEvents: false,
  ...
  function focusPatchObject(patch) {
    ...
    const rings = Array.isArray(patch.geometry?.rings) && patch.geometry.rings.length
      ? patch.geometry.rings
      : [primaryBoundary(patch)];
    const points = rings.flat().filter(p => Number.isFinite(Number(p.lat)) && Number.isFinite(Number(p.lng)));
    if (points.length >= 2) {
      map.fitBounds(points.map(p => [p.lat, p.lng]), { padding: [34, 34], maxZoom: 18 });
    } else if (patch.centroid) {
      map.flyTo([patch.centroid.lat, patch.centroid.lng], Math.max(map.getZoom(), 17), { duration: 0.6 });
    }
  }
  ...
  function openPatchDetail(id, latlng = null) {
    ...
    document.querySelectorAll(".gw-quest-modal-backdrop.gw-patch-detail-backdrop").forEach(el => el.remove());
    ...
    if (latlng && window.L) {
      setTimeout(() => {
        L.popup().setLatLng(latlng).setContent(esc(patchTitle(patch))).openOn(map);
      }, 0);
    }
  }
  ...
  function boundaryLabel(patch) {
    ...
    return pointCount ? `${rings.length} ring${rings.length === 1 ? "" : "s"} / ${pointCount} points` : "No geometry";
  }
  ...
  function osmBoundaryKind(tags = {}) {
    ...
    if (tags.landuse === "grass" || tags.landuse === "meadow") return "grassland";
    return "park";
  }
  ...
  function buildOsmLabelContext(features = [], places = []) {
    ...
    const namedPlaces = places
      ...
      .filter(row => row.name && Number.isFinite(row.point?.lat) && Number.isFinite(row.point?.lng));
  ...
  function osmFeatureLabel(feature, context = {}) {
    ...
    const inheritedName = boundary.length >= 3
      ? inheritedOsmBoundaryName(feature, boundary, context)
      : null;
  ...
  function patchFromOsmFeature(feature, context = {}) {
    ...
    const boundary = feature.points.map(p => ({ lat: Number(p.lat), lng: Number(p.lng) }))
      .filter(p => Number.isFinite(p.lat) && Number.isFinite(p.lng));
  ...
  function nearbyOsmPatchCandidates(limit = 12) {
    ...
    const namedRows = withoutInheritedDuplicates.filter(patch =>
      patch.metadata?.osm_label_source !== "fallback"
    );
  ...
  function openPatchSelector() {
    ...
    root.innerHTML = `
      <div class="gw-quest-modal gw-field-selector-modal">
        ...
        <div class="gw-quest-modal-subtitle">Saved patches and nearby OSM park boundaries.</div>
    ...
    root.onclick = evt => {
      ...
      if (saveBtn) {
        const patch = nearbyOsmPatchCandidates(24).find(row => row.id === saveBtn.dataset.gwSavePatch);
        if (patch) {
          upsertPatch(patch);
          rerender();
        }
  ...
  function renderPatchSelectorRows() {
    ...
    if (empty) {
      return `<div class="gw-muted">No saved patches or loaded OSM park boundaries nearby yet.</div>`;
    }
  ...
  async function fetchProjectDetail(projectOrId) {
    ...
    const data = await fetchINatJson(`https://api.inaturalist.org/v1/projects/${encodeURIComponent(projectId)}`);
    const detail = firstResult(data);
    return detail ? { ...(base || {}), ...detail } : base;
  }

  async function resolveINatProjectBoundary(projectOrId) {
    const key = typeof projectOrId === "object" && projectOrId
  ...
  async function filterProjectsWithBoundaries(projects = [], options = {}) {
    ...
    return rows
      .sort((a, b) => {
        ...
        if (!Number.isFinite(da) && Number.isFinite(db)) return 1;
        return String(a.title || a.name || a.slug || a.id || "")
          .localeCompare(String(b.title || b.name || b.slug || b.id || ""));
      })
  ...
  async function searchINatProjects(query, options = {}) {
    ...
    url.searchParams.set("per_page", String(options.nearby ? INAT_PROJECT_SEARCH_PAGE_SIZE : INAT_PROJECT_RESULT_LIMIT));
  ...
  async function loadINatProjectPatch(input) {
    ...
    const patch = upsertPatch({
      ...
      source_url: project.uri || `https://www.inaturalist.org/projects/${project.slug || project.id || projectId}`,
